/********
Stereo Rectified images + calibration to  ROS Server Publisher
...
{       
	...
	String folder = "/home/jay/image_stitching/opencv_tools/stereo_rect_cam0_png_rgb";
	String folder1 = "/home/jay/image_stitching/opencv_tools/stereo_rect_cam1_png_rgb";
	...
	image_transport::CameraPublisher pub_left = it.advertiseCamera("/stereo/left/image_rect", 1, false);
	image_transport::CameraPublisher pub_right = it.advertiseCamera("/stereo/right/image_rect", 1, false);
	...
	string left_cam_name =  "pgr_camera_13306261";
	string right_cam_name = "pgr_camera_13330289";


	LInfo =	new camera_info_manager::CameraInfoManager (nhleft, left_cam_name, "file:///home/jay/catkin_ws/src/publish_images/calibration/cam0_params.yaml");
	RInfo =	new camera_info_manager::CameraInfoManager (nhright, right_cam_name, "file:///home/jay/catkin_ws/src/publish_images/calibration/cam1_params.yaml");
	...
	while(ros::ok() && i <150)
	{
		Mat left_img = imread(filenames_left[i],CV_LOAD_IMAGE_COLOR);
		Mat right_img = imread(filenames_right[i],CV_LOAD_IMAGE_COLOR);
		...
		header_left.stamp = ros::Time::now(); // time
		left_img_bridge = cv_bridge::CvImage(header_left, sensor_msgs::image_encodings::BGR8, left_img);
		...
		header_right.stamp = ros::Time::now(); // time
		right_img_bridge = cv_bridge::CvImage(header_right, sensor_msgs::image_encodings::BGR8, right_img);
